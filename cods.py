...
class COD:
    ...
    def update_dataset(self, dataset, country, dataset_type, boundary_jsons):
        service_resources = self.get_service_resources(boundary_jsons, country, dataset_type)
        dataset, updated = self.remove_service_resources(dataset)

        if len(service_resources) == 0 and not updated:
            return None

        if len(service_resources) > 0:
            dataset = self.add_service_resources(dataset, service_resources)
